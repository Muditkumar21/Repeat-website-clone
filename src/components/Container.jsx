...
export const Container = forwardRef((props, ref) => {
  const { nodes } = useGLTF("/model/untitledthree.glb");
  ...
  const initialState = useRef({
    ...
    rotation: new THREE.Euler(1.77, 3.29, 0.80),
  ...
  useFrame(() => {
    ...
    const targetRotationX = initialState.current.rotation.x + progress * 1;
    
    // Apply mouse-based rotation (subtle effect)
    // Adjust these multipliers to control the intensity of the effect
    const mouseRotationY = mousePosition.x * 0.15;
    const mouseRotationX = mousePosition.y * 0.15;
    ...
    >
      <group>
        <mesh
          ...
          material={stroke}
        />
        <mesh geometry={nodes.Mesh001_1.geometry} scale={[1.03, 1.05, 1.0]}>
...
useGLTF.preload("/model/untitledthree.glb");
