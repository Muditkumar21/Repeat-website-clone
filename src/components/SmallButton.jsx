...
function SmallButton({ heading, paddingx, paddingy, svgheight, gap, margintop, marginleft, marginright , bg , maintext , arrow , extratext , border , circle }) {
  ...
  return (
    <div>
      <div
        ...
        // bg andborder of main button
        className={`relative ${margintop} ${marginleft} ${marginright} inline-flex justify-center items-center text-base font-main ${bg} border-solid border ${border} rounded-full whitespace-nowrap overflow-hidden`}
      >
        ...
        <div ref={extraTextRef} className={`absolute left-[1.5rem] font-main text-base ${extratext}`}>
          {heading}
        </div>
        <div ref={firstCircleRef} className={`absolute top-[18%] right-[7%] w-[2rem] h-[2rem] rounded-full ${circle}`} />
        <div ref={copyRef} style={{ 
          padding: `${paddingy}rem ${paddingx}rem`, 
          gap: `${gap}rem`,
        }}
        className={`absolute z-[3] left-[-30rem] inline-flex justify-center items-center text-base font-main ${circle} rounded-full whitespace-nowrap`}
        >
          <svg
          ...
          <div className={`${maintext}`}>{heading}</div>
        </div>
        <svg
          ref={mainArrowRef}
          style={{ zIndex: 1, height: `${svgheight}rem` }}
        ...
        <div ref={mainTextRef} className={`z-[1] ${maintext} `}>{heading}</div>
      </div>
    </div>
  );
}
